...
IF NOT EXISTS (SELECT * FROM sys.schemas WHERE name = 'pilot')
    EXEC('CREATE SCHEMA pilot');
...
IF NOT EXISTS (SELECT * FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.name = 'cryptocurrencies' AND s.name = 'pilot')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.name = 'alerts' AND s.name = 'pilot')
BEGIN
